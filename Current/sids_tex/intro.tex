...
promote ``plug-and-play'' CFD.  An agreement was reached to develop CGNS
...
\item Added new quartic elements in \autoref{s:unstructgrid}, according to CPEX 0038.
\end{itemize}
...
\item Modified the description of the \texttt{Elements\_t} structure to incorporate
      the new definition of the \texttt{NGON\_n} element type, and added the
...
\item Added \texttt{ZoneSubRegion\_t} (CPEX 0030) (\autoref{s:ZoneSubRegion}).
      ...
      and (11) addition of \texttt{ZoneGridConnectivityPointers} and
...
\item Added clarity that if rotating about more than one axis, then it is done in a
